...
/* ==========================================================================
   Distribution strip — where the company sits inside its peer cohort

   The quartile box carries the middle half of the cohort, the tick is the
   median, and the gold marker is the subject. This is the picture behind
   every relative grade in the report.
   ========================================================================== */

export function distributionStrip(stat, { fmt = (v) => trim(v, 1), height = 62 } = {}) {
  const svg = frame(height, 'chart--strip');
  if (!stat?.available) return empty(svg, height, 'Not enough peer data to rank this metric');

  const padL = 8, padR = 8;
  const lo = Math.min(stat.min, stat.value);
  const hi = Math.max(stat.max, stat.value);
  const span = hi - lo || Math.abs(hi) || 1;
  const sx = linear(lo - span * 0.06, hi + span * 0.06, padL, W - padR);
  const y = 30, barH = 14;
  ...
  // full range
  g.append(svgEl('line', {
    x1: sx(stat.min), x2: sx(stat.max), y1: y + barH / 2, y2: y + barH / 2,
    stroke: 'var(--chart-grid)', 'stroke-width': 3, 'stroke-linecap': 'round',
  }));
  // interquartile box
  g.append(svgEl('rect', {
    x: sx(stat.q1), y, width: Math.max(sx(stat.q3) - sx(stat.q1), 2), height: barH,
    rx: 3, fill: 'var(--chart-01)', opacity: 0.28,
  }));
  // median tick
  g.append(svgEl('line', {
    x1: sx(stat.median), x2: sx(stat.median), y1: y - 4, y2: y + barH + 4,
    stroke: 'var(--chart-01)', 'stroke-width': 2,
  }));
  g.append(svgEl('text', {
    x: sx(stat.median), y: y + barH + 18, 'text-anchor': 'middle', 'font-size': 10,
  }, `median ${fmt(stat.median)}`));

  // the subject
  const mx = sx(stat.value);
  g.append(svgEl('circle', {
    cx: mx, cy: y + barH / 2, r: 6.5,
    fill: 'var(--brand-01)', stroke: 'var(--surface-1)', 'stroke-width': 2,
  }));
  g.append(svgEl('text', {
    x: mx, y: y - 10, 'text-anchor': 'middle', 'font-size': 11,
    'font-weight': 700, fill: 'var(--text-solid)',
  }, fmt(stat.value)));

  // range ends
  g.append(svgEl('text', { x: sx(stat.min), y: y + barH + 18, 'text-anchor': 'start', 'font-size': 10 }, fmt(stat.min)));
  g.append(svgEl('text', { x: sx(stat.max), y: y + barH + 18, 'text-anchor': 'end', 'font-size': 10 }, fmt(stat.max)));
...
/* ==========================================================================
   Grade bar — a 1-5 factor or metric grade
   ========================================================================== */

export function gradeBar(grade, { height = 8, showScale = false } = {}) {
  const wrap = document.createElement('div');
  wrap.className = 'gradebar';
  const fill = document.createElement('i');
  const pctOf = isNum(grade) ? ((Math.max(1, Math.min(5, grade)) - 1) / 4) * 100 : 0;
  fill.style.width = `${pctOf}%`;
  fill.dataset.tone = !isNum(grade) ? 'na'
    : grade >= 3.4 ? 'good' : grade >= 2.6 ? 'mid' : 'bad';
  wrap.style.setProperty('--gb-h', `${height}px`);
  wrap.append(fill);
  if (showScale) {
    const scale = document.createElement('span');
    scale.className = 'gradebar__scale';
    scale.textContent = '1 · 2 · 3 · 4 · 5';
    wrap.append(scale);
  }
  return wrap;
}
